...
test.beforeEach(async ({ page }, info) => {
  ...
  await page.route(`**/api/admin/sessions/${sessions[0].id}/timeline?*`, route => route.fulfill({ json: adminTimelinePage(sessions[0], timeline, route.request().url()) }));
});
...
test('admin observation opens the first history window on every entry and supports both reading directions', async ({ page }, info) => {
  ...
  for (let entry = 0; entry < 2; entry++) {
    ...
    await page.screenshot({ path: `docs/audits/2026-09-25-admin-redesign-evidence/admin-beginning-${info.project.name}.png` });
...
test('admin session list keeps its width and scroll position when opening and switching detail', async ({ page }, info) => {
  ...
  await page.route('**/api/admin/sessions/extra_*/timeline?*', route => route.fulfill({ json: adminTimelinePage({ ...sessions[0], id: new URL(route.request().url()).pathname.split('/').at(-2) }, [{ id: 'other', kind: 'message', role: 'user', text: 'Selected another audit record' }], route.request().url()) }));
  await openAdminConsole(page, info.project.name);
  for (const width of [1440, 1200, 980]) {
    ...
    await page.screenshot({ path: `docs/audits/2026-09-25-admin-redesign-evidence/admin-fixed-list-${width}.png` });
    await returnToAdminList(page);
  }
});
