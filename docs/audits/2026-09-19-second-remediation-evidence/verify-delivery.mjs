...
const statuses = [];
for (const asset of assets) { const response = await fetch(`${base}${asset}?v=${buildId}`); await response.arrayBuffer(); assert.equal(response.status, 200); statuses.push({ path: asset, status: response.status }); }
